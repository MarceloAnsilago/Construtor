    ...
    return StrategyDocument(
        ...
        signals=SignalsDocument(
            ...
            channels=SignalChannelsDocument(
                enabled=bool(store.get("signals.channels.enabled")),
                indicator=str(store.get("signals.channels.indicator")),
                signal=str(store.get("signals.channels.signal")),
                period=str(store.get("signals.channels.period")),
                deviation=str(store.get("signals.channels.deviation")),
                shift=str(store.get("signals.channels.shift")),
                price=str(store.get("signals.channels.price")),
            ),
        ),
...
def build_runtime_snapshot(store: StrategyStore) -> dict[str, str]:
    return {
        ...
        "signal_channels_enabled": "Sim" if bool(store.get("signals.channels.enabled")) else "Nao",
        "signal_channels_indicator": str(store.get("signals.channels.indicator")),
        "signal_channels_signal": str(store.get("signals.channels.signal")),
        "signal_channels_period": str(store.get("signals.channels.period")),
        "signal_channels_deviation": str(store.get("signals.channels.deviation")),
        "signal_channels_shift": str(store.get("signals.channels.shift")),
        "signal_channels_price": str(store.get("signals.channels.price")),
...
def build_tester_set_lines(store: StrategyStore) -> list[str]:
    ...
    return [
        ...
        f"InpUsarCanaisDeBandas={_bool_to_set(store.get('signals.channels.enabled'))}",
        f"InpIndicadorDosCanais={store.get('signals.channels.indicator')}",
        f"InpSinalDosCanais={store.get('signals.channels.signal')}",
        f"InpPeriodoDosCanais={store.get('signals.channels.period')}",
        f"InpDesvioDosCanais={store.get('signals.channels.deviation')}",
        f"InpDeslocamentoDosCanais={store.get('signals.channels.shift')}",
        f"InpModoDePrecoDosCanais={store.get('signals.channels.price')}",
