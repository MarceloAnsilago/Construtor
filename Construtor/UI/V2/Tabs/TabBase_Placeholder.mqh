...
protected:
   ...
   bool CreatePlaceholder(int window_index,CEF_CTabs &tabs,const int tab_index,const string body_text)
     {
      if(m_created)
         return(true);
      ...
      if(!CreateFrame(m_card,"",tabs,m_window_index,tabs,m_tab_index,content_pad,content_y,content_w,content_h,1))
         return(false);
      m_card.BackColor(C'233,220,203');
      m_card.BorderColor(C'197,168,136');
...
public:
                     CConstrutorV2PlaceholderTab(void) : m_created(false), m_window_index(-1), m_tab_index(-1) {}
  };
